...
using CSCL.Controls.SourceGrid.Utils;
	...
	public class frmSample04 : System.Windows.Forms.Form
	{
		...
		private void btLoad_Click(object sender, System.EventArgs e)
		{
			using (var counter = new PerformanceCounter())
			{
				...
				CSCL.Controls.SourceGrid.Cells.Views.Cell view = new CSCL.Controls.SourceGrid.Cells.Views.Cell();
				view.BackColor = Color.Snow;

				//Editor (IDataModel) shared between all the cells
				CSCL.Controls.SourceGrid.Cells.Editors.TextBox editor = new
					CSCL.Controls.SourceGrid.Cells.Editors.TextBox(typeof(string));
				    ...
				    grid.ColumnsCount > 0)
				{
					...
					for (int r = grid.FixedRows; r < grid.RowsCount; r++)
						grid[r, 0] = new CSCL.Controls.SourceGrid.Cells.RowHeader(r);

					for (int c = grid.FixedColumns; c < grid.ColumnsCount; c++)
					{
						CSCL.Controls.SourceGrid.Cells.ColumnHeader header = new
							CSCL.Controls.SourceGrid.Cells.ColumnHeader(c);
						header.AutomaticSortEnabled = false;
						grid[0, c] = header;
					}
					grid[0, 0] = new CSCL.Controls.SourceGrid.Cells.Header();
				}
				else
				{
					grid.FixedRows = 0;
				...
				for (int r = grid.FixedRows; r < grid.RowsCount; r++)
					for (int c = grid.FixedColumns; c < grid.ColumnsCount; c++)
				{
					grid[r, c] = new CSCL.Controls.SourceGrid.Cells.Cell(r.ToString() + "," +
					                                       c.ToString());
				...
				grid.Selection.Focus(new CSCL.Controls.SourceGrid.Position(0, 0), true);
