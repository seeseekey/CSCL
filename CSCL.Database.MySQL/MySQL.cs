...
namespace CSCL.Database.MySQL
{
	public class MySQL : CSCL.Database.Database
    {
        ...
        public override bool Connect()
        {
            ...
            {
                if(Connected)
                    Disconnect(); //Datenbank schließen falls noch eine geöffnet ist
        ...
        public override void InsertData(DataTable insertData)
        {
            ...
            for(int i=0;i<insertData.Columns.Count;i++)
            {
                ...
                if(i==insertData.Columns.Count-1)
                    sqlCommand+=" \""+insertData.Columns[i]+"\"";
                else
                    sqlCommand+=" \""+insertData.Columns[i]+"\",";
            }
            sqlCommand+=") VALUES";
            ...
            for(int i=0;i<insertData.Rows.Count;i++)
            {
                ...
                for(int j=0;j<insertData.Columns.Count;j++)
                {
                    ...
                    if(j==insertData.Columns.Count-1)
                        sqlCommand+=String.Format("@{0}", insertData.Columns[j].Caption);
                    else
                        sqlCommand+=String.Format("@{0},", insertData.Columns[j].Caption);
                }
        ...
        public override void UpdateData(DataTable updateData, string PrimaryKey)
        {
            ...
            for(int i=0;i<updateData.Rows.Count;i++)
            {
                ...
                for(int j=0;j<updateData.Columns.Count;j++)
                {
                    ...
                    //Parameters zusammenbauen
                    if(j==updateData.Columns.Count-1)
                        sqlCommand+=String.Format("{0}=?", updateData.Columns[j].Caption);
                    else
                        sqlCommand+=String.Format("{0}=?,", updateData.Columns[j].Caption);
                }
        ...
        public override void CreateTable(DataTable table)
        {
            ...
            if(tblName=="")
                throw new Exception("Can't create table whitout name!");
