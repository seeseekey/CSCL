...
namespace CSCL.Controls.SourceGrid
{
	...
	{
		private IComparer m_defaultComparer = new CSCL.Controls.SourceGrid.ValueCellComparer();
		...
		public virtual System.Int32 Compare(System.Object x, System.Object y)
		{
			...
			if (ret == 0)
			{
				CSCL.Controls.SourceGrid.Grid grid = ((CSCL.Controls.SourceGrid.Cells.ICell)x).Grid;
				int rowX = ((CSCL.Controls.SourceGrid.Cells.ICell)x).Range.Start.Row;
				int rowY = ((CSCL.Controls.SourceGrid.Cells.ICell)y).Range.Start.Row;


				int indexOrder = 0;
				while (indexOrder < m_SecondarySortColumns.Length)
				{
					CSCL.Controls.SourceGrid.Cells.ICellVirtual otherX = grid.GetCell(rowX, m_SecondarySortColumns[indexOrder]);
					CSCL.Controls.SourceGrid.Cells.ICellVirtual otherY = grid.GetCell(rowY, m_SecondarySortColumns[indexOrder]);
