...
namespace CSCL.Controls.SourceGrid.Exporter
{
	/// <summary>
	/// Grid print document.
	/// </summary>
	public class GridPrintDocument : PrintDocument
	{
		...
		protected virtual void DrawCell(Graphics g, CellContext ctx, RectangleF rect)
		{
			...
			else
			{
				...
				if ( ctx.Cell.View is CSCL.Controls.SourceGrid.Cells.Views.Cell )
				{
		...
		protected override void OnPrintPage(PrintPageEventArgs e)
		{
			...
			while (m_NextRowToPrint <= RangeToPrint.End.Row)
			{
				...
				while (m_NextColumnToPrint <= pageLastColumn)
				{
					...
					{
						CSCL.Controls.SourceGrid.Cells.ICellVirtual cell = this.m_Grid.GetCell(pos);
						if ( cell != null )
						{
