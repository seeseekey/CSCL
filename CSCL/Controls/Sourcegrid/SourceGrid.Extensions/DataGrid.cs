...
namespace CSCL.Controls.SourceGrid
{
	...
	public class DataGrid : GridVirtual
	{
		...
		{
			CSCL.Controls.SourceGrid.Selection.SelectionBase selObj = base.CreateSelectionObject();

			selObj.EnableMultiSelection = false;
			selObj.FocusStyle = CSCL.Controls.SourceGrid.FocusStyle.RemoveFocusCellOnLeave;
			...
			if (DataSource != null)
			{
				...
				foreach (System.ComponentModel.PropertyDescriptor prop in DataSource.GetItemProperties())
				{
					Columns.Add(prop.Name,
					            prop.DisplayName,
					            CSCL.Controls.SourceGrid.Cells.DataGrid.Cell.Create(prop.PropertyType, !prop.IsReadOnly));
				}
			}
		}
