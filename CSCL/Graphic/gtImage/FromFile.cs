...
namespace CSCL.Graphic
{
	public partial class gtImage
	{
		...
		public static gtImage FromBMPFile(string filename)
		{
			#region Variablen
			// Header
			...
			try
			{
				...
				if(buffer[0]!='B'&&buffer[0]!='M') return new gtImage(); // Signatur überprüfen
				...
				// Konsistenz Check
				if(biPlanes!=1||biWidth<=0||biHeight==0) return new gtImage();
				if(biBitCount!=16&&biBitCount!=32&&(biCompression==BMPBiCompression.BI_ALPHABITFIELDS||
					biCompression==BMPBiCompression.BI_BITFIELDS)) return new gtImage();
				...
				if(biBitCount==32||(biBitCount==16&&(biCompression==BMPBiCompression.BI_ALPHABITFIELDS||
					...
					ret=new gtImage((uint)biWidth, (uint)absHeight, gtImage.Format.RGBA);
				else //Andere Bildern
					ret=new gtImage((uint)biWidth, (uint)absHeight, gtImage.Format.RGB);
				#endregion
				...
				{
					#region RLE 8
					if(biBitCount!=8) return new gtImage();	// nur 8 Bit Bilder erlaubt
				...
				{
					#region RLE 4
					if(biBitCount!=4) return new gtImage(); //Nur 4 Bit Bilder erlaubt
