...
namespace CSCL.Imaging
{
	...
	{
		#region ReduceByN
		public Graphic ReduceBy(uint m, uint n)
		{
			if(m==1&&n==1) return this;
			if((m*n)==0) return new Graphic(0, 0, channelFormat);
		...
		public Graphic ReduceBy(uint m)
		{
			if(m==1) return this;
			if(m==0) return new Graphic(0, 0, channelFormat);
			if((width*height)==0) return new Graphic(0, 0, channelFormat);
		...
		#region Downsampling
		public Graphic Downsample(uint w, uint h)
		{
			if(width<w||height<h)
		...
		unsafe Graphic DownsampleV(uint h)
		{
			if((width*height)==0) return new Graphic(0, 0, channelFormat);
			if(h==0) return new Graphic(0, 0, channelFormat);
